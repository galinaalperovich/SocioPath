...
class Region():
    ...
    def find_node_text(self, node):
        ...
        for des in node.iter():
            try:
                if des.text is not None and (des.tag not in ['script', 'style', 'h1', 'h2'] and not isinstance(des,
                                                                                                               HtmlComment) and re.sub(
                    r"\n|\r|\t| |,|;|\.", "", des.text) != "" or (des.tag == "p")) and des.text.find(
                    ".") > -1 and node_text.find(des.text_content()) == -1:
                    node_text += re.sub(r"\n|\r|\t", "", des.text_content())
                    if des.tag == 'p':
                        node_text += "\n"
